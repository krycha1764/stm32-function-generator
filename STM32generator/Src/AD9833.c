...
void writeSPI(uint16_t word) {
	for (uint8_t i = 0; i < 16 ; i++) {
		...
		for(uint8_t i = 0 ; i < 10; i++) ASM_NOP();
		HAL_GPIO_WritePin(AD9833SCKPORT,AD9833SCK,GPIO_PIN_RESET);             //Data is valid on falling edge
		for(uint8_t i = 0 ; i < 10; i++) ASM_NOP();
		HAL_GPIO_WritePin(AD9833SCKPORT,AD9833SCK,GPIO_PIN_SET);
...
HAL_GPIO_WritePin(AD9833SCKPORT,AD9833SCK,GPIO_PIN_SET);  // Set All SPI pings to High
